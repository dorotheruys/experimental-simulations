...
if __name__ == "__main__":
    ...
    # get_function_from_dataframe(CM_cg_cor_0, 2, 'AoA cor', 'CL_total cor', tunnel_prop_combi_Re, np.linspace(-10, 20, 100), 'AoA', 'CL')

    # 2. CD vs CL with J = 1.6, delta_e = 0
    # get_function_from_dataframe(CM_cg_cor_0, 4, 'CL_total cor', 'CD cor', tunnel_prop_combi_Re, np.linspace(-0.5, 1.5, 100), 'CL', 'CD')

    # 3. Cm vs AoA with J = 1.6, delta_e = 0
    get_function_from_dataframe(CM_cg_cor_0, 5, 'AoA cor', 'CM_total', tunnel_prop_combi_Re[:2], np.linspace(-6, 14, 100), 'AoA', 'CM')
